...
const defaultPlans = [
  ...
  { id: "self_hosted", name: "SELF-HOSTED", title: "自研线路测试套餐", description: "0 元公开测试，仅提供自研线路", traffic: "每 30 天 100G", devices: [1, 1, 1, 1], prices: [0, 0, 0, 0], unlimitedPrices: [0, 0, 0, 0], features: ["自研线路", "实时流量统计", "独立额度控制"], unavailableFeatures: ["不可使用换池功能"], testPlan: true },
]
    ...
    <main className={inAccount ? "px-4 lg:px-6" : "min-h-svh bg-background px-4 py-12 text-foreground md:py-20"}>
      <section className="mx-auto grid max-w-6xl gap-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {plans.map(plan => {
            const isTestPlan = plan.id === "self_hosted"
            const displayedPrice = isTestPlan ? 0 : trafficMode === "unlimited" ? plan.unlimitedPrices[periodIndex] : plan.prices[periodIndex]
            const checkoutOption = isTestPlan ? "self-hosted-test-30" : `${plan.id}${trafficMode === "unlimited" ? "-unlimited" : ""}-${periods[periodIndex].suffix}`
            return <Card key={plan.id} className={plan.recommended ? "border-foreground" : undefined}>
            <CardHeader><div className="flex items-center justify-between"><CardTitle>{plan.name}</CardTitle>{plan.recommended ? <Badge className="bg-emerald-600 text-white dark:bg-emerald-500 dark:text-emerald-950">推荐</Badge> : null}</div><p className="text-sm text-muted-foreground">{plan.title}</p><div className="flex flex-wrap items-baseline gap-2 pt-2"><span><span className="text-4xl font-semibold">￥{displayedPrice}</span><span className="text-sm text-muted-foreground"> / {isTestPlan ? "30天" : periods[periodIndex].days}</span></span>{!isTestPlan && trafficMode === "limited" ? <BillingDiscount monthlyPrice={plan.prices[0]} totalPrice={plan.prices[periodIndex]} months={periods[periodIndex].months} /> : null}</div><p className="text-sm text-muted-foreground sm:min-h-10">{plan.description}</p></CardHeader>
            <CardContent className="grid gap-5"><Button variant={plan.recommended ? "default" : "outline"} asChild>{inAccount ? <Link to={`/account/plans/checkout?option=${checkoutOption}`}>选择套餐</Link> : <Link to="/login?returnTo=/account/plans">登录后购买</Link>}</Button><Separator /><div className="grid gap-3 text-sm"><p className="flex items-center gap-2"><Check className="size-4" />{!isTestPlan && trafficMode === "unlimited" ? "无限流量" : plan.traffic}</p><p className="flex items-center gap-2"><Check className="size-4" />可绑定 {isTestPlan ? plan.devices[0] : plan.devices[periodIndex]} 台设备</p>{plan.features.map(feature => <p key={feature} className="flex items-center gap-2"><Check className="size-4" />{feature}</p>)}{plan.unavailableFeatures.map(feature => <p key={feature} className="flex items-center gap-2 text-muted-foreground"><X className="size-4" /><span className="sr-only">不支持：</span>{feature}</p>)}</div></CardContent>
          </Card>
          })}
        </div>
