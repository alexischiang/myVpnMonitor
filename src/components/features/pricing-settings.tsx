...
function productsFromPricing(pricing: PricingRow[]): ProductItem[] {
  return [
    ...pricing.flatMap(row => {
      ...
      if (["addon", "custom"].includes(row.productKind || "")) return [{ id: `${row.group}:${row.productKind}`, name, description: row.description || row.title || "服务", category: row.productKind === "custom" ? "人工定制" as const : "附加服务" as const, serviceType: row.productKind === "custom" ? "人工定制" as const : "附加服务" as const, billing: row.productKind === "custom" && !row.addonPrice ? "人工报价" : `${formatMoney(Number(row.addonPrice || 0))} / ${row.addonUnit || "次"}`, status: row.enabled === false ? "未开放" as const : "已启用" as const, stock: row.stock, group: row.group, service: row.productKind as "addon" | "custom" }]
      const products: ProductItem[] = row.recurringDeleted ? [] : [{ id: `${row.group}:recurring`, name, description: row.title || row.description || "套餐服务", category: "套餐服务", serviceType: "周期性套餐", billing: `${priceRange(row)} · 30–360天`, status: row.enabled === false ? "未开放" : "已启用", stock: row.stock, group: row.group, service: "recurring" }]
      if (row.lifetimeDeleted !== true && Number.isFinite(Number(row.lifetimePrice))) products.push({ id: `${row.group}:lifetime`, name: row.lifetimeName || `${name} 不限时`, description: row.lifetimeDescription || "固定流量不限时长", category: "套餐服务", serviceType: "不限时套餐", billing: `${formatMoney(Number(row.lifetimePrice))} · ${row.lifetimeTraffic || "固定流量"}，不限时长`, status: row.lifetimeEnabled === false ? "未开放" : "已启用", stock: row.lifetimeStock, group: row.group, service: "lifetime" })
      return products
    }),
  ]
}
...
export function PricingDetailPage() {
  ...
  if (!row) return <div className="grid gap-4 px-4 lg:px-6"><EmptyState title="商品不存在" description="请返回商品管理重新选择。" /><Button asChild variant="outline" className="w-fit"><Link to="/pricing-settings"><ArrowLeft />返回商品管理</Link></Button></div>
  ...
  return <div className="grid gap-4 px-4 lg:px-6">
    ...
    {!isService && isLifetime ? <Card><CardHeader><CardTitle>交付线路</CardTitle><CardDescription>不限时套餐统一由 3x-ui 自研线路交付，并按固定总流量自动配置用户额度。</CardDescription></CardHeader></Card> : null}
  </div>
}
