...
export function CatalogV2ProductsPage() {
  ...
  return <div className="grid gap-4 px-4 lg:px-6">
    <PageHeader title="新版商品录入" description="V2 商品数据暂不参与商城展示和支付，录入完成后再执行旧商品迁移。" />
    {error ? <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert> : null}
    <div className="flex flex-wrap gap-2"><Button asChild><Link to="/catalog-v2/products/new"><Plus />新建商品</Link></Button><Button variant="outline" asChild><Link to="/xui-inbounds">线路权限组</Link></Button><Button variant="outline" asChild><Link to="/pricing-settings">查看旧商品</Link></Button></div>
...
export function CatalogV2ProductDetailPage() {
  ...
  return <div className="grid gap-4 px-4 lg:px-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><PageHeader title={isNew ? "新建 V2 商品" : product.name} description="此处数据暂不参与现有商城和支付链路。" /><div className="flex gap-2"><Button variant="outline" asChild><Link to="/catalog-v2"><ArrowLeft />返回</Link></Button>{isNew ? null : <Button variant="destructive" onClick={() => void remove()} disabled={saving}><Trash2 />删除</Button>}<Button onClick={() => void save()} disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Save />}保存商品</Button></div></div>
