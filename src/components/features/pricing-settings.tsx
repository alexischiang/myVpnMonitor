...
function productsFromPricing(pricing: PricingRow[]): ProductItem[] {
  return [
    ...pricing.flatMap(row => {
      ...
      const products: ProductItem[] = row.recurringDeleted ? [] : [{ id: `${row.group}:recurring`, name, description: row.title || row.description || "套餐服务", category: "套餐服务", serviceType: "周期性套餐", billing: `${priceRange(row)} · 30–360天`, status: row.availability?.recurring ? "已启用" : "未开放", stock: row.stock, group: row.group, service: "recurring" }]
