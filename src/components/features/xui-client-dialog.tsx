    ...
    <DialogContent>
      <form className="grid gap-5" onSubmit={submit}>
        ...
        {mode === "import" ? <Item variant="outline">
          <ItemContent><ItemTitle>{preview.email}</ItemTitle><ItemGroup className="mt-2 grid-cols-2"><Item variant="muted" size="sm"><ItemContent><ItemDescription>流量限额</ItemDescription><ItemTitle>{formatBytes(preview.totalBytes)}</ItemTitle></ItemContent></Item><Item variant="muted" size="sm"><ItemContent><ItemDescription>在线IP数量</ItemDescription><ItemTitle>{preview.limitIp || "不限"}</ItemTitle></ItemContent></Item><Item variant="muted" size="sm"><ItemContent><ItemDescription>到期日</ItemDescription><ItemTitle>{preview.expiresAt ? formatDate(preview.expiresAt) : "不限"}</ItemTitle></ItemContent></Item><Item variant="muted" size="sm"><ItemContent><ItemDescription>流量重置日</ItemDescription><ItemTitle>每月 {preview.resetDay} 日</ItemTitle></ItemContent></Item></ItemGroup></ItemContent>
        </Item> : selectedClient ? <Item variant="outline">
          <ItemContent><ItemTitle>{selectedClient.email}</ItemTitle><ItemDescription>关联后邮箱设为 {user.email} · 3x-ui 原额度 {selectedClient.totalBytes ? formatBytes(selectedClient.totalBytes) : "不限"}，将按 App 当前套餐额度 {preview.totalBytes ? formatBytes(preview.totalBytes) : "不限流量"} 覆盖 · 在线IP数量 {selectedClient.limitIp || "不限"} · {selectedClient.expiryTime ? formatDate(new Date(selectedClient.expiryTime).toISOString()) : "永不过期"}</ItemDescription></ItemContent>
        </Item> : null}
