...
export function PricingDetailPage() {
  ...
  return <div className="grid gap-4 px-4 lg:px-6">
    ...
    <header className="flex flex-wrap items-start justify-between gap-4"><section className="grid gap-1"><h2 className="text-xl font-semibold tracking-tight">{isNew ? "新增商品" : serviceName}</h2><p className="text-sm text-muted-foreground">{isCustom ? "人工确认需求、价格和交付内容的定制服务。" : isAddon ? "在周期套餐基础上购买的附加服务。" : isLifetime ? "固定流量、不限使用时长的独立商品。" : "按月、季、半年或年度续期的独立商品。"}点击保存后生效。</p></section><div className="flex flex-wrap gap-2">{isNew ? null : <Button variant="destructive" onClick={() => setDeleteOpen(true)} disabled={saving || deleting}><Trash2 />删除</Button>}<BackButton fallback="/pricing-settings" /><Button onClick={() => void save()} disabled={saving || deleting}>{saving ? <Loader2 className="animate-spin" /> : <Save />}{saving ? "保存中..." : "保存配置"}</Button></div></header>
